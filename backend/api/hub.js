...
router.use(session({secret: "", saveUninitialized: true, resave: false, cookie: {maxAge: 120000}}));

router.post("/login", (req, res) => {
    // req.body.password
    // req.body.email
    database.query(`SELECT password FROM librarians WHERE email = ${req.body.email}`, (err, rows) => {
        if (err) throw err
        else {
            if (rows.length == 1) {
                compare(req.body.password, Buffer.from(rows[0].password, "binary").toString(), (err, same) => {
                    if (err) throw err
                    else if (same) {
